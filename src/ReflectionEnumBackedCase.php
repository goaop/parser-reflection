...
use ReflectionEnumBackedCase as InternalReflectionEnumBackedCase;
use UnitEnum;
...
final class ReflectionEnumBackedCase extends InternalReflectionEnumBackedCase
{
    ...
    /**
     * Initializes a reflection for an enum backed case
     */
    public function __construct(
        ...
        if ($enumCaseNode->expr !== null) {
            $expressionSolver = new NodeExpressionResolver($this->enumReflection);
            $expressionSolver->process($enumCaseNode->expr);
            $value = $expressionSolver->getValue();
            if (!is_int($value) && !is_string($value)) {
                throw new ReflectionException("Backing value for $className::$caseName must be int or string");
            }
            $this->backingValue = $value;
        } else {
            throw new ReflectionException("Backed enum case $className::$caseName must have a value expression");
        }
    }
     ...
     * Consider using {@see getBackingValue()} instead if you only need the scalar backing value.
     */
